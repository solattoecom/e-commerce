...
Preciso de um ecommerce para venda de calçados, primeiramente, vamos modelar a página inicial, onde ao entrar, 3 opções de login serão apresentadas, com a animação de um sapato surgindo de fundo. **PROMPT:**
...
` — "Ready to Transfer
Without Borders?" with classes `font-normal leading-[1.1] mb-[15px]` and inline `fontSize: '3.5rem', letterSpacing: '-0.03em'`.
- `

` — "Send Money Worldwide at the Best Rates" with `text-[0.9rem] mb-[30px] font-normal opacity-85`.
- `` — "Get Started Today", classes `bg-neutral-900 text-white font-semibold cursor-pointer border-none text-[0.95rem] transition-all duration-200 hover:-translate-y-0.5`, inline `padding: '14px 32px', borderRadius: '12px', boxShadow: '0 10px 20px rgba(0,0,0,0.3)'`. On hover, bump shadow to `0 14px 30px rgba(0,0,0,0.4)` via `onMouseEnter`/`onMouseLeave`.
...
State: `const [activeIndex, setActiveIndex] = useState(0);` with toggle function.
...
`, container `max-w-[1100px] w-full mx-auto px-5`.
...
` of `Features, Benefits, Testimonials, Pricing` — each `
...
Add to `index.html` ``: Google Fonts Inter preconnect + stylesheet link:
```html
...
Continue developing this project in the [Lovable editor](https://lovable.dev/projects/70e9f10f-12f3-4a30-a409-a32a63017933).
